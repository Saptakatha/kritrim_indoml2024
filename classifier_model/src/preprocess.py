...
def preprocess_fn(fname: str, data_dir: Optional[str] = "data") -> None:
    ...
    with open(file, 'r') as f:
        for line in tqdm(f):
            ...
            pattern = r'"([^"]+)":\s?(?:(\d+(?:\.\d+)?)|"([^"]+)")' # match key: value or key: "value"
            ...
            for attr in attrs:
                ...
                if key == 'indoml_id':
                    ...
                    value = str(float(attr[1])) if attr[1] != '' else attr[2].strip().replace('\"', '')
                datapoint[key] = value
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_dir", default="data", help="The directoy to store data in.")
