...
from transformers import BertTokenizerFast, BertForSequenceClassification, AutoTokenizer, AutoModelForSeq2SeqLM
...
class Trainer:
    ...
    def __init__(self, data_dir, device, output="brand", trim=False, learning_rate=3e-3):
        ...
        # self.tokenizer = AutoTokenizer.from_pretrained('/rec-data/sapta/misc/phase_2/indoml2024/flan_t5-small')
        # self.lm = AutoModelForSeq2SeqLM.from_pretrained('/rec-data/sapta/misc/phase_2/indoml2024/flan_t5-small')
        self.lm.eval()
        ...
        # to save best models
        if not os.path.exists("model/") and self.device == 0:
            os.makedirs("model/")
    ...
    def train(self, num_epochs=5):
        ...
        for epoch in range(num_epochs):
            ...
            if f1 > best_f1:
                ...
                if ((self.ddp and self.device == 0) or not self.ddp):
                    torch.save(best_model, f"model/best_model_{self.output}_epoch_{epoch+1}.pt")
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="brand",
                        choices=["supergroup", "group", "module", "brand"])
